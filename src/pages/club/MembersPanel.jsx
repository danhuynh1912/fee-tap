...
export function MembersPanel({ club, members, plan, pollTally, hostName, hostAvatar, currentUserId, canEdit, onChanged, onHitLimit, toast }) {
  ...
  async function add(e) {
    ...
    setBusy(true)
    try {
      const { data: newMember, error } = await supabase.from('club_members').insert({ club_id: club.id, name: name.trim() }).select().single()
  ...
  return (
    <Card>
      ...
      <ul className="mt-4 space-y-2">
        ...
        {count === 0 && !hostName && (
          <li className="rounded-2xl border border-dashed border-slate-200 py-10 text-center text-sm text-slate-400">{t('mem_empty')}</li>
        )}
