...
export function CourtSlotModal({ slot, onSave, onClose }) {
  ...
  function handleSave() {
    if (!form.weekdays.length) {
      setError(t('slot_no_weekdays'))
      return
    }
    if (!form.venue_name.trim() && !form.name.trim()) {
      setError(t('slot_name'))
      return
    }
    setError('')
  ...
  const preview = previewSlot.weekdays.length && num(form.price_per_hour) > 0 ? computeSlot(previewSlot) : null
  ...
  return (
    <Modal open onClose={onClose} maxW="max-w-2xl">
      <h2 className="text-lg font-bold text-slate-900 mb-5">{isEdit ? t('slot_edit') : t('slot_add')}</h2>
      ...
      <div className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Tên cụm sân" icon={MapPin} hint="Dùng để nhóm các sân trong cùng địa điểm">
            <input
              className={inputCls}
              placeholder="vd: Sân ABC, Nhà thi đấu Thể công"
          ...
          {!isEdit && (
            <Field label="Số sân" icon={Layers} hint="Tạo nhiều slot cùng lúc cho các sân giống nhau">
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => setCourtCount((c) => Math.min(10, c + 1))}
                  className="h-10 w-10 shrink-0 rounded-xl border border-slate-200 bg-slate-50 text-lg font-bold text-slate-700 hover:bg-slate-100 transition"
                >
                  +
                </button>
              </div>
              {n > 1 && <p className="mt-1.5 text-xs text-slate-400">Tạo: {Array.from({ length: n }, (_, i) => `Sân ${i + 1}`).join(', ')}</p>}
            </Field>
          )}
        </div>
        ...
        {(isEdit || n === 1) && (
          <Field label={t('slot_name')} icon={Hash} hint={isEdit ? undefined : 'Để trống sẽ dùng tên cụm sân'}>
            <input
              className={inputCls}
              placeholder={form.venue_name || t('slot_name_ph')}
              value={form.name}
          ...
          <Field label={t('slot_price')} icon={Coins}>
            <div className="relative">
              <input
                ...
                value={fmtInputNum(form.price_per_hour)}
                onChange={(e) => set('price_per_hour', parseInputNum(e.target.value))}
              />
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label={t('slot_billing_cycle')} icon={RefreshCw} hint={cycleLabelShort(parseInt(form.cycle_months, 10) || 1, i18n.language)}>
            <div className="relative">
              <input
                type="number"
                min="1"
                ...
                onChange={(e) => set('cycle_months', e.target.value)}
              />
        ...
        {parseInt(form.cycle_months, 10) > 1 && (
          <Field label={t('slot_cycle_start')} icon={Calendar}>
            <select className={inputCls} value={form.cycle_start_month} onChange={(e) => set('cycle_start_month', e.target.value)}>
        ...
        {preview && (
          <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 space-y-1.5 animate-fade-in">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{formatPeriodLabel(preview.period, i18n.language)}</p>
            <div className="flex justify-between text-sm">
              <span className="text-slate-500">{t('set_cost_sessions')}</span>
              <span className="font-mono font-bold text-slate-900">
                {fmtNum(preview.totalSessions)}
                {n > 1 ? ` × ${n} sân` : ''}
              </span>
            </div>
            <div className="flex justify-between text-sm">
            ...
            {preview.nextDeadline && (
              <div className="flex justify-between text-xs text-slate-500 pt-1 border-t border-slate-100">
                ...
                <span className="font-semibold">{preview.nextDeadline.toLocaleDateString(i18n.language === 'vi' ? 'vi-VN' : 'en-US')}</span>
              </div>
            )}
          </div>
        )}
