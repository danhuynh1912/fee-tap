...
function MemberRow({ member, paymentRecord, onConfirmManual, confirmingId }) {
  const { t } = useTranslation()
  const isPaid = paymentRecord?.status === 'paid' || paymentRecord?.status === 'manual'
  const isConfirming = confirmingId === member.id

  return (
    <div className={cx('flex items-center gap-2.5 rounded-2xl border px-3 py-2.5 sm:px-4 sm:py-3', isPaid ? 'border-lime-200 bg-lime-50' : 'border-slate-100 bg-white')}>
      {/* Avatar */}
...
export function PaymentCollectionModal({ open, onClose, collection, memberPayments, members, toast, onChanged, onSynced }) {
  const { t } = useTranslation()
