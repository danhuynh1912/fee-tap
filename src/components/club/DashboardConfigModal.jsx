...
export function DashboardConfigModal({ open, onClose, clubId, sections, onSaved }) {
  ...
  async function save() {
    ...
    const { data: verify } = await supabase.from('club_settings').select('dashboard_sections').eq('club_id', clubId).maybeSingle()
        ...
        {SECTIONS.map(({ key, labelKey, adminOnly }) => {
          ...
          return (
            ...
            >
              ...
              <span className={cx('shrink-0 text-[10px] font-bold uppercase tracking-wide', visible ? 'text-lime-600' : 'text-slate-400')}>
                {visible ? 'ON' : 'OFF'}
              </span>
            </button>
          )
